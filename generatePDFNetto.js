...
export function generatePDFNetto(data, additionalInfo) {
  const element = document.createElement("div");

  let productRows = data.products
    .map((product) => {
      const netPrice = calculateNetPrice(product.grossPrice);
      const sumNetPrice = netPrice * parseInt(product.quantity);
      return `
      <tr>
        <td><img src="${
          product.image
...
<table id="pdfGenerator">
  <tr>
    ...
    <th>Suma netto</th>
        ...
        ${productRows}
          <tr>
    <td colspan="5">Koszt transportu</td>
